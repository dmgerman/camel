...
begin_comment
comment|/**  * Integration test requires running Zipkin running  *  *<p>The easiest way to run is locally:  *<pre>{@code  * curl -sSL https://zipkin.io/quickstart.sh | bash -s  * java -jar zipkin.jar  * }</pre>  *  *<p>This test has to be run with environment variables set. For example:  *<pre>{@code  * ZIPKIN_COLLECTOR_HTTP_SERVICE_HOST=localhost  * ZIPKIN_COLLECTOR_HTTP_SERVICE_PORT=9411  * }</pre>  */
end_comment

begin_class
DECL|class|ZipkinAutoConfigureHttp
specifier|public
class|class
name|ZipkinAutoConfigureHttp
extends|extends
name|CamelTestSupport
block|{
DECL|field|zipkin
...
comment|// NOTE: this won't work unless you have an auto-configuration file in src/test/resources
