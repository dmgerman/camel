...
block|}
block|}
DECL|method|sendAsynchronous (final Exchange exchange, final HttpClient client, final JettyContentExchange httpExchange)
specifier|protected
name|void
name|sendAsynchronous
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// the copy must be an in ouy MEP
name|copy
operator|.
name|setPattern
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// configure http exchange to signal when its complete
name|httpExchange
operator|.
...
argument_list|)
expr_stmt|;
comment|// now we need to let the original exchange to stop
comment|// and let that copy exchange continue
...
name|debug
argument_list|(
literal|"Continue to route Exchange: "
operator|+
name|exchange
argument_list|)
expr_stmt|;
block|}
comment|// TODO: hook into exiting route path
name|exchange
operator|.
name|getContext
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// this is only needed if we are asynchronous where we need to have a thread pool of listeners
comment|// that will process the completed tasks
if|if
condition|(
operator|!
name|getEndpoint
