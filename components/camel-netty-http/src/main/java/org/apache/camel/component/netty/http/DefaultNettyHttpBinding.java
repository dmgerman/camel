...
condition|)
block|{
comment|// the body should hide the stacktrace (muteException enabled)
name|body
operator|=
name|NettyConverter
operator|.
...
argument_list|)
expr_stmt|;
comment|// force content type to be text/plain
name|message
operator|.
name|setHeader
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// and mark the exception as failure handled, as we handled it by actively muting it
name|ExchangeHelper
operator|.
name|setFailureHandled
argument_list|(
...
return|;
block|}
comment|/*      * set the HTTP status code      */
DECL|method|determineResponseCode (Exchange camelExchange, Object body)
specifier|private
name|int
name|determineResponseCode
parameter_list|(
...
comment|// request received by the proxy, only when the body wasn't
comment|// modified by a processor on route
if|if
condition|(
name|givenRequest
operator|!=
