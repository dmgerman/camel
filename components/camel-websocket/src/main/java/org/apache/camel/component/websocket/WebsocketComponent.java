...
argument_list|)
expr_stmt|;
comment|// TODO -  Is it the right place to define static resources ?
if|if
condition|(
name|endpoint
operator|.
...
argument_list|)
expr_stmt|;
comment|// Create ServletContextHandler and add it to the Jetty server
name|context
operator|=
name|createContext
argument_list|(
...
expr_stmt|;
block|}
comment|// TODO - chm - 25/05
comment|// As we can define WebSocket for Consumer/Producer
comment|// This part of the code must be adapted compare to camel-jetty where we only use
comment|// Jetty as a server = Consumer
comment|// connectorRef.servlet.connect(consumer);
block|}
block|}
...
block|{
comment|// TODO - chm - 25/05
comment|// Why do we have to do a setConsumer on the Servlet ?
name|servlet
operator|.
name|setConsumer
argument_list|(
