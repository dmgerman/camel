...
argument_list|)
expr_stmt|;
comment|// check if the file can be uploaded
comment|// otherwise upload a new revision of the existing file
name|folder
operator|.
name|canUpload
argument_list|(
...
parameter_list|)
block|{
comment|// https://developer.box.com/en/reference/options-files-content/
comment|// error 409, filename exists, get the file info to upload a new revision
if|if
condition|(
literal|409
operator|==
...
condition|)
block|{
comment|// the box search api relies on a folder index to search for items, however our tests
comment|// noticed the folder (box service) can take 11 minutes to reindex
comment|// for a recently uploaded file, the folder search will return empty as the file was not indexed yet.
comment|// see https://community.box.com/t5/Platform-and-Development-Forum/Box-Search-Delay/m-p/40072
comment|// this check operation uses the folder list item as fallback mechanism to check if the file really exists
comment|// it is slower than the search api, but reliable
comment|// for faster results, we recommend the folder to contain no more than 500 items
comment|// otherwise it can take more time to iterate over all items to check if the filename exists
comment|// display a WARN if the delay is higher than 5s
name|long
name|init
init|=
name|System
...
control|)
block|{
comment|// check if the filename exists
name|exists
operator|=
literal|"file"
operator|.
...
name|warn
argument_list|(
literal|"The upload operation, checks if the file exists by using the Box list folder, however it took "
operator|+
name|elapsed
operator|+
literal|" seconds to verify, try to reduce the size of the folder items for faster results."
argument_list|)
expr_stmt|;
block|}
if|if
