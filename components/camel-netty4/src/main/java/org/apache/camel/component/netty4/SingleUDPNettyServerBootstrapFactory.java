...
argument_list|)
expr_stmt|;
comment|// TODO need to find the right setting of below option
comment|// only set this if user has specified
comment|/*         if (configuration.getReceiveBufferSizePredictor()> 0) {             bootstrap.setOption("receiveBufferSizePredictorFactory",                     new FixedReceiveBufferSizePredictorFactory(configuration.getReceiveBufferSizePredictor()));         }*/
if|if
condition|(
name|configuration
operator|.
...
expr_stmt|;
block|}
comment|//TODO need to check the additional netty options
comment|/*         if (configuration.getOptions() != null) {             for (Map.Entry<String, Object> entry : configuration.getOptions().entrySet()) {                 connectionlessBootstrap.setOption(entry.getKey(), entry.getValue());             }         }*/
name|LOG
operator|.
name|debug
argument_list|(
literal|"Created ConnectionlessBootstrap {}"
argument_list|,
name|bootstrap
argument_list|)
expr_stmt|;
