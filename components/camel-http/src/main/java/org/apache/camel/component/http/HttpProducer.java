...
condition|)
block|{
comment|// join multi-values separated by semi-colon
name|httpRequest
operator|.
name|addHeader
argument_list|(
...
condition|)
block|{
comment|// close the stream at the end of the exchange to ensure it gets eventually closed later
name|exchange
operator|.
...
argument_list|)
decl_stmt|;
comment|// special for HTTP DELETE/GET if the message body should be included
if|if
condition|(
name|getEndpoint
argument_list|()
...
condition|)
block|{
comment|// only create entity for http payload if the HTTP method carries payload (such as POST)
name|HttpEntity
name|requestEntity
init|=
name|createRequestEntity
