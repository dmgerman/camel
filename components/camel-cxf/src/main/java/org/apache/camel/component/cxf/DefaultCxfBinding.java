...
begin_comment
comment|/**  * The Default CXF binding implementation.  */
end_comment
...
block|}
block|}
comment|/**      * This method is called by {@link CxfConsumer} to populate a CXF response protocol headers       * from a Camel exchange headers before CheckError.      * Ensure can send protocol headers back even error/exception thrown      */
DECL|method|populateCxfHeaderFromCamelExchangeBeforeCheckError (Exchange camelExchange, org.apache.cxf.message.Exchange cxfExchange)
specifier|public
name|void
...
condition|)
block|{
comment|// We need to keep the Content-Type if the data format is RAW message
name|DataFormat
name|dataFormat
init|=
name|exchange
...
return|;
block|}
DECL|method|getContentTypeString (List<String> values)
specifier|protected
name|String
name|getContentTypeString
parameter_list|(
...
continue|continue;
block|}
comment|// We need to copy the content-type if the dataformat is RAW
if|if
condition|(
name|Message
operator|.
...
control|)
block|{
comment|// We should not override the namespace setting of the element
if|if
condition|(
name|XMLConstants
operator|.
...
condition|)
block|{
comment|// if there is no Node and the addNamespaceContext option is enabled, this map is available
name|nsMap
operator|.
name|putAll
argument_list|(
...
return|;
block|}
DECL|method|getXMLString (Element el)
specifier|private
specifier|static
name|String
name|getXMLString
