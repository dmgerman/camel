...
expr_stmt|;
block|}
comment|//if not specify the service name and if the wsdlUrl is available,
comment|//parse the wsdl to see if only one service in it, if so set the only service
...
condition|)
block|{
comment|// use wsdl manager to parse wsdl or get cached
comment|// definition
try|try
block|{
name|Definition
...
annotation|@
name|Override
DECL|method|processResult (Message message, org.apache.cxf.message.Exchange exchange, BindingOperationInfo oi, Map<String, Object> resContext)
specifier|protected
name|Object
index|[]
name|processResult
...
parameter_list|)
block|{
comment|//Camel does not strickly enforce returning a value when a value is required from the WSDL/contract
comment|//Thus, we'll capture the exception raised and return a null
return|return
literal|null
return|;
block|}
...
expr_stmt|;
block|}
comment|/**      * get the request uri for a given exchange.      */
DECL|method|getRequestUri (Exchange camelExchange)
name|URI
name|getRequestUri
parameter_list|(
name|Exchange
...
name|error
argument_list|(
literal|"cannot determine request URI"
argument_list|,
name|e
argument_list|)
expr_stmt|;
