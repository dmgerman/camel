...
name|latch
decl_stmt|;
DECL|field|failFastAssertionError
specifier|private
specifier|volatile
name|AssertionError
name|failFastAssertionError
decl_stmt|;
...
control|)
block|{
comment|// skip tasks which we have already been running in fail fast mode
name|boolean
name|skip
init|=
name|failFast
...
expr_stmt|;
block|}
DECL|method|getAssertPeriod ()
specifier|public
name|long
name|getAssertPeriod
parameter_list|()
block|{
return|return
name|assertPeriod
...
expr_stmt|;
block|}
comment|/**      * Adds an expectation that the given property values are received by this      * endpoint in any order.      *<p/>      *<b>Important:</b> The number of values must match the expected number of messages, so if you expect 3 messages, then      * there must be 3 values.      *<p/>      *<b>Important:</b> This overrides any previous set value using {@link #expectedMessageCount(int)}      */
DECL|method|expectedPropertyValuesReceivedInAnyOrder (final String name, final List<?> values)
specifier|public
name|void
name|expectedPropertyValuesReceivedInAnyOrder
parameter_list|(
specifier|final
name|String
name|name
...
operator|)
operator|+
literal|" properties. Expected property values: "
operator|+
name|actualPropertyValues
argument_list|,
name|actualPropertyValues
...
block|}
comment|/**      * Adds an expectation that the given property values are received by this      * endpoint in any order      *<p/>      *<b>Important:</b> The number of values must match the expected number of messages, so if you expect 3 messages, then      * there must be 3 values.      *<p/>      *<b>Important:</b> This overrides any previous set value using {@link #expectedMessageCount(int)}      */
DECL|method|expectedPropertyValuesReceivedInAnyOrder (String name, Object... values)
specifier|public
name|void
name|expectedPropertyValuesReceivedInAnyOrder
parameter_list|(
...
block|}
block|}
DECL|method|assertMessagesSorted (Expression expression, boolean ascending, int index)
specifier|protected
name|void
name|assertMessagesSorted
parameter_list|(
...
block|}
block|}
comment|/**      * Asserts among all the current received exchanges that there are no duplicate message      *      * @param expression  the expression to use for duplication check      */
...
expr_stmt|;
block|}
DECL|method|getResultMinimumWaitTime ()
specifier|public
name|long
name|getResultMinimumWaitTime
parameter_list|()
block|{
return|return
name|resultMinimumWaitTime
...
return|;
block|}
comment|/**      * Sets whether to make a deep copy of the incoming {@link Exchange} when received at this mock endpoint.      *<p/>      * Is by default<tt>true</tt>.      */
DECL|method|setCopyOnExchange (boolean copyOnExchange)
specifier|public
name|void
...
return|;
block|}
comment|/**      * Sets whether {@link #assertIsSatisfied()} should fail fast      * at the first detected failed expectation while it may otherwise wait for all expected      * messages to arrive before performing expectations verifications.      *      * Is by default<tt>true</tt>. Set to<tt>false</tt> to use behavior as in Camel 2.x.      */
DECL|method|setFailFast (boolean failFast)
specifier|public
name|void
name|setFailFast
parameter_list|(
...
condition|)
block|{
comment|// fail fast mode so check n'th expectations as soon as possible
name|int
name|index
init|=
name|getReceivedCounter
...
control|)
block|{
comment|// only assertion tasks can support fail fast mode
if|if
condition|(
name|test
operator|instanceof
...
name|debug
argument_list|(
literal|"Running assertOnIndex({}) on task: {}"
argument_list|,
name|index
argument_list|,
name|task
...
name|e
expr_stmt|;
comment|// signal latch we are done as we are failing fast
name|log
operator|.
name|debug
argument_list|(
literal|"Assertion failed fast on "
operator|+
name|index
operator|+
literal|" received exchange due to "
operator|+
name|e
operator|.
name|getMessage
