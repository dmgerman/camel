...
begin_comment
comment|/**  * A builder of expressions or predicates based on values.  *<p/>  * This implementation is a derived copy of the<tt>org.apache.camel.builder.ValueBuilder</tt> from camel-core,  * that are specialized for being used with the mock component and separated from camel-core.  */
end_comment

begin_class
DECL|class|MockValueBuilder
specifier|public
class|class
...
name|not
decl_stmt|;
DECL|method|MockValueBuilder (Expression expression)
specifier|public
name|MockValueBuilder
parameter_list|(
name|Expression
...
parameter_list|()
block|{
comment|// chicken-and-egg situation as we need to return an ExpressionClause
comment|// which needs a right-hand side that is being built via the fluent
comment|// builder that is returned, and therefore we need to use a ref
comment|// to the expression (right hand side) that will be used below
comment|// in the onNewPredicate where the actual matching is executed
specifier|final
name|AtomicReference
argument_list|<
name|Expression
...
block|}
decl_stmt|;
comment|// okay now we can set the reference to the right-hand-side
name|ref
operator|.
name|set
argument_list|(
