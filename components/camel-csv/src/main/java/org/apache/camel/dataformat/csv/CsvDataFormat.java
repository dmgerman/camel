...
name|strategy
init|=
name|cloneCSVStrategyIfNecessary
argument_list|(
name|CSVStrategy
operator|.
...
name|useMaps
decl_stmt|;
DECL|method|cloneCSVStrategyIfNecessary (CSVStrategy csvStrategy)
specifier|private
specifier|static
name|CSVStrategy
name|cloneCSVStrategyIfNecessary
...
condition|)
block|{
comment|// return a safe copy of the static constant so that we don't cause any side effect
comment|// by the other CsvDataFormat objects in case we would change any property on this
comment|// strategy itself (e.g. it's delimiter through the #unmarshal() method below)
name|LOGGER
operator|.
name|debug
argument_list|(
literal|"Returning a clone of {} as it is the declared constant {} by the CSVStrategy class"
argument_list|,
name|csvStrategy
argument_list|,
name|field
...
block|}
block|}
DECL|method|loadAllAsList (CsvIterator<T> iter)
specifier|private
parameter_list|<
name|T
parameter_list|>
...
return|;
block|}
comment|/**      * Sets whether or not the result of the unmarshalling should be a {@code java.util.Map} instead of a {@code java.util.List}. It uses the first line as a      * header line and uses it as keys of the maps.      *      * @param useMaps {@code true} in order to use {@code java.util.Map} instead of {@code java.util.List}, {@code false} otherwise.      */
