...
name|configAdminInitBundle
operator|=
name|createConfigAdminInitBundle
argument_list|(
name|configAdminPidFiles
argument_list|)
expr_stmt|;
...
return|;
block|}
DECL|method|createBundleContext (String name, String bundleFilter, TinyBundle bundle, TinyBundle configAdminInitBundle, ClassLoader loader)
specifier|public
specifier|static
name|BundleContext
name|createBundleContext
...
argument_list|)
decl_stmt|;
comment|// let's put configadmin before blueprint.core
name|int
name|idx1
init|=
operator|-
...
if|if
condition|(
literal|"org.apache.aries.blueprint.core"
operator|.
name|equals
argument_list|(
name|bd
...
block|}
block|}
comment|/**      * Synchronization method to wait for particular state of BlueprintContainer under test.      */
DECL|method|waitForBlueprintContainer (final Set<Long> eventHistory, BundleContext context, final String symbolicName, final int bpEvent, final Runnable runAndWait)
specifier|public
specifier|static
name|void
name|waitForBlueprintContainer
...
condition|)
block|{
comment|// we skip events that we've already seen
comment|// it works with BP container reloads if next CREATE state is at least 1ms after previous one
if|if
condition|(
name|eventHistory
operator|==
...
condition|)
block|{
comment|// we didn't wait for FAILURE, but we got it - fail fast then
name|pThrowable
index|[
literal|0
index|]
...
name|RuntimeException
argument_list|(
literal|"Gave up waiting for BlueprintContainer from bundle \""
operator|+
name|symbolicName
operator|+
literal|"\""
...
name|set
argument_list|(
literal|"X-Camel-Blueprint-ConfigAdmin-Init"
argument_list|,
name|configAdminInit
operator|.
name|toString
...
DECL|method|getBundleDescriptors (final String bundleFilter, ClassLoader loader)
specifier|private
specifier|static
name|List
argument_list|<
...
return|;
block|}
comment|/**      * Bundle activator that will be invoked in right time to set initial configadmin configuration      * for blueprint container.      */
DECL|class|TestBundleActivator
specifier|public
specifier|static
class|class
name|TestBundleActivator
...
argument_list|()
decl_stmt|;
comment|// we are started before blueprint.core and felix.configadmin
comment|// we are sure that felix.configadmin is started before blueprint.core
name|sysContext
operator|.
...
condition|)
block|{
comment|// configadmin should have already been started
name|ServiceReference
argument_list|<
name|?
argument_list|>
...
argument_list|(
literal|"org.osgi.service.cm.ConfigurationAdmin"
argument_list|)
decl_stmt|;
if|if
condition|(
...
block|}
block|}
DECL|method|initializeConfigAdmin (BundleContext context, String configAdminInit)
specifier|private
name|void
name|initializeConfigAdmin
parameter_list|(
