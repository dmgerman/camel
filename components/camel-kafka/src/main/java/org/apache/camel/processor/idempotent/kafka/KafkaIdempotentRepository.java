...
begin_comment
comment|/**  * A Kafka topic-based implementation of {@link org.apache.camel.spi.IdempotentRepository}.  *  * Uses a local cache of previously seen Message IDs. All mutations of the cache are via a Kafka topic, on which  * additions and removals are broadcast. The topic used must be unique per repository instance. This class makes no  * assumptions about number of partitions (it is designed to consume from all at the same time), or replication factor.  * Each repository instance that uses the topic (e.g. on different machines running in parallel) controls its own  * consumer group.  *  * On startup, the instance subscribes to the topic and rewinds the offset to the beginning, rebuilding the cache to the  * latest state.  *  * To use, this repository must be placed in the Camel registry, either manually or by registration as a bean in  * Spring/Blueprint, as it is CamelContext aware.  *  * @author jkorab  */
end_comment
...
expr_stmt|;
block|}
DECL|method|KafkaIdempotentRepository (String topic, String bootstrapServers, int maxCacheSize)
specifier|public
name|KafkaIdempotentRepository
parameter_list|(
name|String
...
expr_stmt|;
block|}
DECL|method|KafkaIdempotentRepository (String topic, Properties consumerConfig, Properties producerConfig, int maxCacheSize)
specifier|public
name|KafkaIdempotentRepository
parameter_list|(
name|String
...
argument_list|)
expr_stmt|;
comment|// set up the producer to remove all batching on send
name|producerConfig
operator|.
name|putIfAbsent
argument_list|(
...
argument_list|)
decl_stmt|;
DECL|method|TopicPoller (Consumer<String, String> consumer, CountDownLatch cacheReadyLatch)
name|TopicPoller
parameter_list|(
name|Consumer
argument_list|<
