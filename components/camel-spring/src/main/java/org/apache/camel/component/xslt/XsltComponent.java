...
name|Exception
block|{
comment|// force to load the resource on each exchange as we are not cached
name|loadResource
argument_list|(
name|xslt
argument_list|,
...
block|{
comment|// we have already loaded xslt so we are cached
return|return
operator|new
name|ProcessorEndpoint
argument_list|(
...
block|}
block|}
DECL|method|loadResource (XsltBuilder xslt, Resource resource)
specifier|private
name|void
name|loadResource
parameter_list|(
...
condition|)
block|{
comment|// prefer to use file when a file based url
name|File
name|file
init|=
name|resource
...
condition|)
block|{
comment|// check if the file exists and report a better error as the XSLT
comment|// will just say it cannot compile the stylesheet file
if|if
condition|(
operator|!
name|file
...
block|{
comment|// fallback and use input stream
name|xslt
operator|.
