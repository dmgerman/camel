...
begin_comment
comment|/**  * A file read lock that uses an {@link IdempotentRepository} and {@link FileRenameExclusiveReadLockStrategy rename} as the lock strategy.  * This allows to plugin and use existing idempotent repositories that for example supports clustering.  * The other read lock strategies that are using marker files or file locks, are not guaranteed to work in clustered setup with various platform and file systems.  */
end_comment

begin_class
DECL|class|FileIdempotentRenameRepositoryReadLockStrategy
specifier|public
class|class
name|FileIdempotentRenameRepositoryReadLockStrategy
extends|extends
name|ServiceSupport
...
name|removeOnCommit
decl_stmt|;
DECL|method|FileIdempotentRenameRepositoryReadLockStrategy ()
specifier|public
name|FileIdempotentRenameRepositoryReadLockStrategy
parameter_list|()
block|{
...
condition|)
block|{
comment|// if we acquired during idempotent then check rename also
name|answer
operator|=
name|rename
operator|.
