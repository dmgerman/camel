...
expr_stmt|;
block|}
comment|// copy over any available trace entity (i.e. Segment) from the old thread to the new one
comment|// according to AWS XRay documentation on multithreading:
comment|// https://docs.aws.amazon.com/xray/latest/devguide/xray-sdk-java-multithreading.html
comment|//
comment|//> If you use multiple threads to handle incoming requests, you can pass the current segment or subsegment
comment|//> to the new thread and provide it to the global recorder. This ensures that the information recorded
comment|//> within the new thread is associated with the same segment as the rest of the information recorded about
comment|//> that request.
name|Entity
name|entity
init|=
literal|null
