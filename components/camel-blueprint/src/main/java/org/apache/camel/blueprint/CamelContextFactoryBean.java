...
begin_comment
comment|/**  * A bean to create and initialize a {@link BlueprintCamelContext}  * and install routes either explicitly configured in  * Blueprint XML or found by searching the classpath for Java classes which extend  * {@link RouteBuilder} using the nested {@link #setPackages(String[])}.  *  * @version $Revision: 813776 $  */
end_comment
...
comment|//            if (beanPostProcessor instanceof ApplicationContextAware) {
comment|//                ((ApplicationContextAware)beanPostProcessor).setApplicationContext(applicationContext);
comment|//            }
comment|//            if (beanPostProcessor instanceof CamelBeanPostProcessor) {
comment|//                ((CamelBeanPostProcessor)beanPostProcessor).setCamelContext(getContext());
comment|//            }
comment|//        }
...
literal|null
decl_stmt|;
comment|//        String[] names = getApplicationContext().getBeanNamesForType(clazz, true, true);
comment|//        if (names.length == 1) {
comment|//            bean = (T) getApplicationContext().getBean(names[0], clazz);
comment|//        }
comment|//        if (bean == null) {
comment|//            ApplicationContext parentContext = getApplicationContext().getParent();
comment|//            if (parentContext != null) {
comment|//                names = parentContext.getBeanNamesForType(clazz, true, true);
comment|//                if (names.length == 1) {
comment|//                    bean = (T) parentContext.getBean(names[0], clazz);
comment|//                }
comment|//            }
comment|//        }
return|return
...
expr_stmt|;
block|}
comment|//    public void onApplicationEvent(ApplicationEvent event) {
comment|//        if (context != null) {
comment|//            // let the spring camel context handle the events
comment|//            context.onApplicationEvent(event);
comment|//        } else {
comment|//            if (LOG.isDebugEnabled()) {
comment|//                LOG.debug("Publishing spring-event: " + event);
comment|//            }
comment|//
comment|//            if (event instanceof ContextRefreshedEvent) {
comment|//                // now lets start the CamelContext so that all its possible
comment|//                // dependencies are initialized
comment|//                try {
comment|//                    LOG.debug("Starting the context now!");
comment|//                    getContext().start();
comment|//                } catch (Exception e) {
comment|//                    throw wrapRuntimeCamelException(e);
comment|//                }
comment|//            }
comment|//        }
comment|//    }
...
return|;
block|}
DECL|method|setContext (BlueprintCamelContext context)
specifier|public
name|void
name|setContext
parameter_list|(
...
expr_stmt|;
block|}
comment|//    public ApplicationContext getApplicationContext() {
comment|//        if (applicationContext == null) {
comment|//            throw new IllegalArgumentException("No applicationContext has been injected!");
comment|//        }
comment|//        return applicationContext;
comment|//    }
comment|//
comment|//    public void setApplicationContext(ApplicationContext applicationContext) {
comment|//        this.applicationContext = applicationContext;
comment|//    }
...
return|;
block|}
comment|/**      * Sets the package names to be recursively searched for Java classes which      * extend {@link RouteBuilder} to be auto-wired up to the      * {@link BlueprintCamelContext} as a route. Note that classes are excluded if      * they are specifically configured in the spring.xml      *      * @deprecated replaced by {@link #setPackageScan(org.apache.camel.model.PackageScanDefinition)}      * @param packages the package names which are recursively searched      */
annotation|@
name|Deprecated
...
expr_stmt|;
block|}
comment|//    public void setBeanPostProcessor(BeanPostProcessor postProcessor) {
comment|//        this.beanPostProcessor = postProcessor;
comment|//    }
comment|//
comment|//    public BeanPostProcessor getBeanPostProcessor() {
comment|//        return beanPostProcessor;
comment|//    }
...
argument_list|()
decl_stmt|;
comment|//        BlueprintCamelContext ctx = new BlueprintCamelContext(getApplicationContext());
name|ctx
operator|.
name|setName
argument_list|(
...
argument_list|)
decl_stmt|;
comment|//            RouteBuilderFinder finder = new RouteBuilderFinder(getContext(), normalized, getContextClassLoaderOnStart(),
comment|//                    getBeanPostProcessor(), getContext().getPackageScanClassResolver());
comment|//            finder.appendBuilders(getAdditionalBuilders());
block|}
block|}
