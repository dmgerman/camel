...
begin_comment
comment|/**  * The jms component allows messages to be sent to (or consumed from) a JMS Queue or Topic.  *  * This component uses Spring JMS and supports JMS 1.1 and 2.0 API.  */
end_comment
...
name|Service
block|{
DECL|field|runningMessageListeners
specifier|private
specifier|final
name|AtomicInteger
name|runningMessageListeners
init|=
operator|new
name|AtomicInteger
argument_list|()
...
block|}
block|}
comment|// now configure the JMS 2.0 API
if|if
condition|(
name|configuration
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"Invalid Endpoint configuration: "
operator|+
name|consumer
operator|.
name|getEndpoint
...
name|replyTo
operator|+
literal|" cannot be the same as the destination name on the JmsConsumer as that"
operator|+
literal|" would lead to the consumer sending reply messages to itself in an endless loop."
argument_list|)
throw|;
block|}
return|return
...
expr_stmt|;
block|}
DECL|method|getDestinationType ()
specifier|public
name|String
...
return|;
block|}
comment|/**      * Name of the queue or topic to use as destination      */
...
return|;
block|}
DECL|method|onListenerContainerStarting (AbstractMessageListenerContainer container)
specifier|public
name|void
...
name|trace
argument_list|(
literal|"There are still {} running message listeners. Cannot stop endpoint {}"
argument_list|,
name|running
argument_list|,
name|this
...
name|trace
argument_list|(
literal|"There are still {} running message listeners. Cannot shutdown endpoint {}"
argument_list|,
name|running
argument_list|,
name|this
...
name|description
operator|=
literal|"Number of running message listeners"
argument_list|)
DECL|method|getRunningMessageListeners ()
specifier|public
name|int
name|getRunningMessageListeners
parameter_list|()
block|{
return|return
name|runningMessageListeners
