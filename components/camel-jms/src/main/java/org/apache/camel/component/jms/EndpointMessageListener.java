...
name|disableReplyTo
expr_stmt|;
comment|// we should also not send back reply to ourself if this destination and replyDestination is the same
name|Destination
name|destination
init|=
...
name|debug
argument_list|(
literal|"JMSDestination and JMSReplyTo is the same, will skip sending a reply message to itself: {}"
argument_list|,
name|destination
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// any problems with eager loading then set an exception so Camel error handler can react
name|exchange
operator|.
name|setException
argument_list|(
