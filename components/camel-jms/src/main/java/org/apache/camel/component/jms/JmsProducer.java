...
name|debug
argument_list|(
literal|"Using {}: {}, JMSReplyTo destination: {}, with request timeout: {} ms."
argument_list|,
operator|new
name|Object
index|[]
...
block|}
block|}
DECL|method|resolveOrCreateDestination (String destinationName, Session session)
specifier|protected
name|Destination
name|resolveOrCreateDestination
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// okay then fallback and create the queue/topic
if|if
condition|(
name|isPubSub
condition|)
...
decl_stmt|;
name|ScheduledExecutorService
name|replyManagerScheduledExecutorService
init|=
name|getEndpoint
argument_list|()
operator|.
...
name|name
operator|=
literal|"JmsReplyManagerOnTimeout["
operator|+
name|getEndpoint
argument_list|()
operator|.
...
literal|"]"
expr_stmt|;
comment|// allow the timeout thread to timeout so during normal operation we do not have a idle thread
name|int
name|max
init|=
name|getEndpoint
...
name|IllegalArgumentException
argument_list|(
literal|"The option replyToOnTimeoutMaxConcurrentConsumers must be>= 1"
argument_list|)
throw|;
block|}
name|ExecutorService
