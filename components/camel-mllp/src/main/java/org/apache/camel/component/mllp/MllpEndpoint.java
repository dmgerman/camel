...
begin_comment
comment|/**  * Provides functionality required by Healthcare providers to communicate with other systems using the MLLP protocol.  *<p/>  * MLLP payloads are not logged unless the logging level is set to DEBUG or TRACE to avoid introducing PHI  * into the log files. Logging of PHI can be globally disabled by setting the org.apache.camel.mllp.logPHI system  * property to false.  */
end_comment
...
return|;
block|}
comment|/**      * The maximum number of timeouts (specified by receiveTimeout) allowed before the TCP Connection will be reset.      *      * @param maxReceiveTimeouts maximum number of receiveTimeouts      */
DECL|method|setMaxReceiveTimeouts (int maxReceiveTimeouts)
specifier|public
name|void
...
return|;
block|}
comment|/**      * Enable/Disable the validation of HL7 Payloads      *      * If enabled, MLLP Payloads are buffered and written to the external system in a single write(byte[]) operation.      * If disabled, the MLLP payload will not be buffered, and three write operations will be used.  The first operation      * will write the MLLP start-of-block character {0x0b (ASCII VT)}, the second operation will write the HL7 payload, and the      * third operation will writh the MLLP end-of-block character and the MLLP end-of-data character {[0x1c, 0x0d] (ASCII [FS, CR])}.      *      * @param bufferWrites enabled if true, otherwise disabled      */
