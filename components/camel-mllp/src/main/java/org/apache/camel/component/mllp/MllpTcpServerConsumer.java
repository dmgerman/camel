...
name|description
operator|=
literal|"MllpTcpServer Consumer"
argument_list|)
...
name|DefaultConsumer
block|{
DECL|field|SOCKET_STARTUP_TEST_WAIT
specifier|public
specifier|static
specifier|final
name|int
...
literal|100
decl_stmt|;
DECL|field|SOCKET_STARTUP_TEST_READ_TIMEOUT
specifier|public
specifier|static
specifier|final
name|int
...
name|description
operator|=
literal|"Check server connection"
argument_list|)
...
name|description
operator|=
literal|"Starts serverSocket thread and waits for requests"
argument_list|)
DECL|method|startMllpConsumer ()
specifier|public
name|void
name|startMllpConsumer
parameter_list|()
...
name|description
operator|=
literal|"Stops client threads and serverSocket thread"
argument_list|)
DECL|method|stopMllpConsumer ()
specifier|public
name|void
name|stopMllpConsumer
parameter_list|()
...
name|debug
argument_list|(
literal|"Socket appears to be there - checking for available data in {} milliseconds"
argument_list|,
name|SOCKET_STARTUP_TEST_WAIT
argument_list|)
expr_stmt|;
...
name|log
argument_list|,
literal|"Preparing to check for available data on component startup"
argument_list|)
expr_stmt|;
try|try
block|{
...
name|debug
argument_list|(
literal|"Check for available data failed - Socket.read() returned END_OF_STREAM"
argument_list|)
expr_stmt|;
name|MllpSocketUtil
operator|.
...
init|=
literal|"Check for available data failed - Socket.read() timed-out after {} milliseconds."
operator|+
...
name|debug
argument_list|(
literal|"Ignoring IOException encountered when attempting to read a byte - connection was reset"
argument_list|)
expr_stmt|;
try|try
block|{
...
name|debug
argument_list|(
literal|"Ignoring IOException encountered when attempting to close the connection after the connection reset was detected"
argument_list|,
name|closeEx
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**      * Nested Class reads the Socket      */
...
name|log
argument_list|,
literal|"Constructing ClientSocketThread"
argument_list|)
expr_stmt|;
name|mllpSocketReader
operator|=
...
condition|)
block|{
comment|// No data received - check for max timeouts
if|if
condition|(
name|endpoint
operator|.
...
name|format
argument_list|(
literal|"Idle Client after %d receive timeouts [%d-milliseconds] - resetting connection"
argument_list|,
name|receiveTimeoutCounter
argument_list|,
name|endpoint
...
name|warn
argument_list|(
literal|"Exception encountered reading payload - sending exception to route"
argument_list|,
name|mllpEx
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception encountered processing exchange with exception encounter reading payload"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|format
argument_list|(
literal|"Exchange property %s is %b"
argument_list|,
name|MLLP_RESET_CONNECTION_BEFORE_SEND
argument_list|,
name|exchange
...
comment|// TODO:  Enhance this to say whether or not the acknowledgment is missing or just of an un-convertible type
name|byte
index|[]
...
argument_list|)
expr_stmt|;
comment|// TODO:  Use the charset of the exchange
name|message
operator|.
name|setHeader
argument_list|(
