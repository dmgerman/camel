...
condition|)
block|{
comment|// Encode the private key in PEM format for JSCH
name|StringBuilder
name|sb
init|=
operator|new
...
name|warn
argument_list|(
literal|"PrivateKey in the KeyPair must be filled"
argument_list|)
expr_stmt|;
block|}
block|}
...
expr_stmt|;
block|}
name|createResultHeadersFromExchange
argument_list|(
literal|null
argument_list|,
name|exchange
...
return|;
block|}
comment|/**      * Helper method which gets result code and message from sftpException and puts it into header.      * In case that exception is null, it sets successfull response.      */
DECL|method|createResultHeadersFromExchange (SftpException sftpException, Exchange exchange)
specifier|private
name|void
name|createResultHeadersFromExchange
parameter_list|(
...
parameter_list|)
block|{
comment|//if exception is null, it means that result was ok
if|if
condition|(
name|sftpException
operator|==
