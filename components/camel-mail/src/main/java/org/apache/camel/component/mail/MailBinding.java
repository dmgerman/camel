...
expr_stmt|;
block|}
comment|// fix content type to include a space after semi colon if missing
if|if
condition|(
name|contentType
operator|!=
...
argument_list|)
decl_stmt|;
comment|// after is the charset lets see if its given and a valid charset
if|if
condition|(
name|after
operator|!=
...
return|;
block|}
DECL|method|determineCharSet (MailConfiguration configuration, String charset)
specifier|protected
name|String
name|determineCharSet
parameter_list|(
...
name|charset
operator|+
literal|" is not supported, will fallback to use platform default instead."
argument_list|)
expr_stmt|;
return|return
literal|null
...
block|}
comment|/**      * Extracts the body from the Mail message      */
DECL|method|doExtractBodyFromMail (Exchange exchange, Message message, boolean firstAttempt)
specifier|protected
name|Object
name|doExtractBodyFromMail
parameter_list|(
