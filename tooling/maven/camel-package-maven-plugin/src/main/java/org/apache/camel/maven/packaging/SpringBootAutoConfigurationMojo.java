...
control|)
block|{
comment|// skip some components which is duplicates
name|boolean
name|skip
init|=
name|Arrays
...
block|}
block|}
comment|/*             // only create source code if the component has options that can be used in auto configuration             if (!model.getComponentOptions().isEmpty()) {                  // use springboot as sub package name so the code is not in normal                 // package so the Spring Boot JARs can be optional at runtime                 int pos = model.getJavaType().lastIndexOf(".");                 String pkg = model.getJavaType().substring(0, pos) + ".springboot";                  createComponentConfigurationSource(pkg, model, overrideComponentName);                 createComponentAutoConfigurationSource(pkg, model, Collections.singletonList(componentName));                 createComponentSpringFactorySource(pkg, model);             }             */
block|}
block|}
