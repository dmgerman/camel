...
block|}
block|}
comment|// validate all the options
name|boolean
name|first
init|=
literal|true
...
condition|)
block|{
comment|// skip any leading double slash in first property from uri as that is from the scheme part
if|if
condition|(
name|value
operator|!=
...
block|}
block|}
comment|// is reference lookup of bean (not applicable for @UriPath, or multi-valued)
if|if
condition|(
operator|!
name|mulitValue
...
return|;
block|}
DECL|method|doValidateSimple (String simple, boolean predicate)
specifier|private
name|SimpleValidationResult
name|doValidateSimple
parameter_list|(
