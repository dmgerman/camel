...
name|isOneWay
decl_stmt|;
DECL|method|JMSOutputStream (Message m)
specifier|public
name|JMSOutputStream
parameter_list|(
name|Message
...
comment|//inMessage.put(JMSConstants.CAMEL_CLIENT_RESPONSE_HEADERS,
comment|//              outMessage.get(JMSConstants.CAMEL_CLIENT_RESPONSE_HEADERS));
comment|/*             Object result1;              Object result = null;              javax.jms.Message jmsMessage1 = pooledSession.consumer().receive(timeout);             getLogger().log(Level.FINE, "client received reply: " , jmsMessage1);              if (jmsMessage1 != null) {                  base.populateIncomingContext(jmsMessage1, outMessage, JMSConstants.CAMEL_CLIENT_RESPONSE_HEADERS);                 String messageType = jmsMessage1 instanceof TextMessage                             ? JMSConstants.TEXT_MESSAGE_TYPE : JMSConstants.BINARY_MESSAGE_TYPE;                 result = base.unmarshal((org.apache.camel.Exchange) outMessage);                 result1 = result;             } else {                 String error = "JMSClientTransport.receive() timed out. No message available.";                 getLogger().log(Level.SEVERE, error);                 //TODO: Review what exception should we throw.                 throw new JMSException(error);              }             response = result1;              //set the message header back to the incomeMessage             inMessage.put(JMSConstants.CAMEL_CLIENT_RESPONSE_HEADERS,                           outMessage.get(JMSConstants.CAMEL_CLIENT_RESPONSE_HEADERS));              */
name|getLogger
argument_list|()
operator|.
name|log
