...
begin_comment
comment|/**  * Represents the kind of options for what to do with the current task when shutting down.  *<p/>  * By default the current task is allowed to complete. However some consumers such as  * {@link BatchConsumer} have pending tasks which you can configure the consumer   * to complete as well.  *<ul>  *<li>CompleteCurrentTaskOnly - Is the<b>default</b> behavior where a route consumer will be shutdown as fast as  *                                 possible. Allowing it to complete its current task, but not to pickup pending  *                                 tasks (if any).</li>  *<li>CompleteAllTasks - Allows a route consumer to continue to complete all pending tasks (if any).</li>   *                            *</ul>  *<b>Notice:</b> Most consumers only have a single task, but {@link org.apache.camel.BatchConsumer} can have  * many tasks and thus this option mostly applies to this kind of consumer.  */
end_comment
