...
parameter_list|)
block|{
comment|// iterator to walk all nodes
specifier|final
name|Iterator
argument_list|<
name|ProcessorDefinition
...
argument_list|)
decl_stmt|;
comment|// iterator to only walk nodes which matchBy matches
specifier|final
name|Iterator
argument_list|<
name|ProcessorDefinition
...
parameter_list|()
block|{
comment|// grab the next if its ready
if|if
condition|(
name|next
operator|!=
...
return|;
block|}
comment|// find the next which matchBy matches
name|boolean
name|found
init|=
literal|false
...
block|}
decl_stmt|;
comment|// iterator to only walk which selectXXX matches
return|return
operator|new
name|Iterator
argument_list|<
...
return|;
block|}
comment|// a bit complicated logic to ensure selectFirst/selectLast,selectFrom/selectTo
comment|// filter out unwanted nodes
comment|// we use the matchBy iterator as the nodes mush at first match this iterator
comment|// before we can do any selection
if|if
condition|(
name|selectFrom
operator|>=
...
condition|)
block|{
comment|// spool until we should start
while|while
condition|(
name|current
operator|<=
...
condition|)
block|{
comment|// spool until the last matching
while|while
condition|(
name|itMatchBy
operator|.
...
condition|)
block|{
comment|// regular without any selectFirst,selectLast,selectFrom/selectTo stuff
name|current
operator|++
expr_stmt|;
if|if
