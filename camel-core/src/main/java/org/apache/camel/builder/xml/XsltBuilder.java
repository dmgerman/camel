...
literal|true
decl_stmt|;
DECL|field|setMessageEmitterMethod
specifier|private
specifier|volatile
name|Method
name|setMessageEmitterMethod
decl_stmt|;
DECL|field|saxonReceiverClass
specifier|private
specifier|volatile
name|Class
argument_list|<
name|?
argument_list|>
name|saxonReceiverClass
decl_stmt|;
DECL|field|saxonWarnerClass
specifier|private
specifier|volatile
name|Class
argument_list|<
...
argument_list|()
decl_stmt|;
comment|// special for saxon as we need to call setMessageEmitter on the transformer to hook from saxon to the JAXP errorListener
comment|// so we can get notified if any errors happen during transformation
comment|// see details at: https://stackoverflow.com/questions/4695489/capture-xslmessage-output-in-java
if|if
condition|(
name|isSaxonTransformer
argument_list|(
...
condition|)
block|{
comment|// set our error listener so we can capture errors and report them back on the exchange
name|transformer
operator|.
name|setErrorListener
argument_list|(
...
block|{
comment|// use custom error listener
name|transformer
operator|.
name|setErrorListener
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// create a transformer to see if its saxon, as we then need to do some initial preparation
name|Transformer
name|t
init|=
name|getTemplate
...
condition|)
block|{
comment|// pre-load saxon classes as we need to call the setMessageEmitter on the transformer to hook saxon to use the JAXP
comment|// error listener, so we can capture errors and xsl:message outputs which end users may define in the xslt files
try|try
block|{
name|saxonReceiverClass
operator|=
...
argument_list|(
literal|"net.sf.saxon.event.Receiver"
argument_list|)
expr_stmt|;
name|saxonWarnerClass
operator|=
...
argument_list|(
literal|"Error pre-loading Saxon classes. Make sure you have saxon on the classpath,"
operator|+
literal|" and the classloader can load the following two classes: net.sf.saxon.event.Receiver, net.sf.saxon.serialize.MessageWarner."
argument_list|,
name|e
argument_list|)
throw|;
