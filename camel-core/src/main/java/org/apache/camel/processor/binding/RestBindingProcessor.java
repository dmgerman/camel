...
name|wasXml
decl_stmt|;
DECL|method|RestBindingMarshalOnCompletion (AsyncProcessor jsonMmarshal, AsyncProcessor xmlMmarshal, boolean wasXml)
specifier|private
name|RestBindingMarshalOnCompletion
parameter_list|(
name|AsyncProcessor
...
parameter_list|)
block|{
comment|// only marshal if we succeeded
...
argument_list|)
expr_stmt|;
comment|// TODO: add logic to detect what content-type is now
comment|// also when we add support for @Produces then use that to determine if we should marshal to xml or json
try|try
block|{
if|if
condition|(
