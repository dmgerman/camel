...
return|;
block|}
DECL|method|doProcessParallel (final Exchange original, final AtomicReference<Exchange> result, final Iterable<ProcessorExchangePair> pairs, final boolean streaming, final AsyncCallback callback)
specifier|protected
name|void
name|doProcessParallel
parameter_list|(
...
name|executionException
decl_stmt|;
DECL|method|AggregateOnTheFlyTask (AtomicReference<Exchange> result, Exchange original, AtomicInteger total, CompletionService<Exchange> completion, AtomicBoolean running, CountDownLatch aggregationOnTheFlyDone, AtomicBoolean allTasksSubmitted, AtomicReference<Exception> executionException)
specifier|private
name|AggregateOnTheFlyTask
parameter_list|(
name|AtomicReference
...
condition|)
block|{
name|ParallelAggregateTimeoutTask
name|task
init|=
operator|new
name|ParallelAggregateTimeoutTask
...
block|{
comment|// in non parallel mode then just run the task
name|task
operator|.
name|run
argument_list|()
...
DECL|class|ParallelAggregateTask
specifier|private
specifier|final
class|class
name|ParallelAggregateTask
...
name|aggregated
decl_stmt|;
DECL|method|ParallelAggregateTask (AtomicReference<Exchange> result, Exchange subExchange, AtomicInteger aggregated)
specifier|private
name|ParallelAggregateTask
parameter_list|(
name|AtomicReference
...
block|}
comment|/**      * Worker task to aggregate the old and new exchange on-the-fly for completed tasks when using parallel processing.      */
DECL|class|ParallelAggregateTimeoutTask
specifier|private
specifier|final
class|class
name|ParallelAggregateTimeoutTask
...
name|timedOut
decl_stmt|;
DECL|method|ParallelAggregateTimeoutTask (Exchange original, AtomicReference<Exchange> result, CompletionService<Exchange> completion, AtomicInteger aggregated, AtomicInteger total, AtomicBoolean timedOut)
specifier|private
name|ParallelAggregateTimeoutTask
parameter_list|(
name|Exchange
...
expr_stmt|;
block|}
comment|// we timed out so increment the counter
name|aggregated
operator|.
name|incrementAndGet
argument_list|()
...
block|}
block|}
DECL|method|doProcessSequential (Exchange original, AtomicReference<Exchange> result, Iterable<ProcessorExchangePair> pairs, AsyncCallback callback)
specifier|protected
name|boolean
name|doProcessSequential
parameter_list|(
...
return|;
block|}
DECL|method|doProcessSequential (final Exchange original, final AtomicReference<Exchange> result, final Iterable<ProcessorExchangePair> pairs, final Iterator<ProcessorExchangePair> it, final ProcessorExchangePair pair, final AsyncCallback callback, final AtomicInteger total)
specifier|private
name|boolean
name|doProcessSequential
parameter_list|(
