...
name|ExecutionException
block|{
comment|// execute tasks in parallel and aggregate in the order they are finished (out of order sequence)
name|CompletionService
argument_list|<
name|Exchange
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// add to the list of ordered exchanges to aggregate when all the tasks is finished
name|ordered
operator|.
name|add
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// increment number of tasks to do
name|latch
operator|.
name|increment
argument_list|()
...
expr_stmt|;
block|}
comment|// this task is done so decrement
name|latch
operator|.
name|decrement
argument_list|()
...
expr_stmt|;
block|}
comment|// wait for all tasks to be complete
name|latch
operator|.
name|await
argument_list|()
expr_stmt|;
comment|// aggregate in order
for|for
control|(
name|Exchange
name|subExchange
