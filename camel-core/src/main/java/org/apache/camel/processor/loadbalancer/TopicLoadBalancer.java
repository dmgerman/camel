...
begin_comment
comment|/**  * A {@link LoadBalancer} implementations which sends to all destinations  * (rather like JMS Topics).   *  It is deprecated in Camel 2.0, you need to move to use multicast,   *  if you want to send the message to all destinations.  *   * @version $Revision$  * @deprecated  */
end_comment
...
parameter_list|)
block|{
comment|// We don't handle the exception here
block|}
block|}
name|callback
operator|.
