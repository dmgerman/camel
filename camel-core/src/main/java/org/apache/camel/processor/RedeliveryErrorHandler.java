...
continue|continue;
block|}
comment|// do a sanity check whether we are still allowed to run before continuing as we just woke up
if|if
condition|(
operator|!
name|isRunAllowed
...
block|}
block|}
comment|/**      * Strategy the detect if we are allowed to run      *<p/>      * This implementation detects if Camel is shutting down as well      *      * @param exchange  the exchange      * @return<tt>true</tt> if we can process the exchange,<tt>false</tt> to stop processing it      */
DECL|method|isRunAllowed (Exchange exchange)
specifier|protected
name|boolean
name|isRunAllowed
parameter_list|(
...
parameter_list|)
block|{
comment|// check if camel is stopping
name|boolean
name|stoppingCamel
init|=
literal|false
...
name|debug
argument_list|(
literal|"Rejected execution as we are stopping for exchange: "
operator|+
name|exchange
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|// and stop continue routing
name|exchange
operator|.
name|setProperty
argument_list|(
