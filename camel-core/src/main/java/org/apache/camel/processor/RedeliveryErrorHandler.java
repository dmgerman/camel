...
parameter_list|)
block|{
comment|// Create the redelivery state object for this exchange
...
return|;
block|}
comment|/**      * Contains the current redelivery state      */
DECL|class|RedeliveryState
specifier|protected
class|class
name|RedeliveryState
implements|implements
...
name|RejectedExecutionException
argument_list|(
literal|"Redelivery not allowed while stopping"
argument_list|)
argument_list|)
expr_stmt|;
comment|// mark the exchange as redelivery exhausted so the failure processor / dead letter channel can process the exchange
name|exchange
operator|.
name|setProperty
argument_list|(
...
return|;
block|}
DECL|method|isRedeliveryAllowed ()
specifier|protected
name|boolean
name|isRedeliveryAllowed
parameter_list|()
...
expr_stmt|;
block|}
DECL|method|prepareExchangeForContinue (Exchange exchange, boolean isDeadLetterChannel)
specifier|protected
name|void
...
expr_stmt|;
block|}
DECL|method|prepareExchangeForRedelivery ()
specifier|protected
name|void
name|prepareExchangeForRedelivery
parameter_list|()
...
DECL|method|onExceptionOccurred ()
specifier|protected
name|void
name|onExceptionOccurred
parameter_list|()
...
DECL|method|deliverToOnRedeliveryProcessor ()
specifier|protected
name|void
name|deliverToOnRedeliveryProcessor
parameter_list|()
...
DECL|method|deliverToFailureProcessor (final Processor processor, final boolean isDeadLetterChannel, final Exchange exchange)
specifier|protected
name|void
name|deliverToFailureProcessor
parameter_list|(
...
expr_stmt|;
block|}
DECL|method|prepareExchangeAfterFailure (final Exchange exchange, final boolean isDeadLetterChannel, final boolean shouldHandle, final boolean shouldContinue)
specifier|protected
name|void
name|prepareExchangeAfterFailure
parameter_list|(
...
block|}
block|}
DECL|method|logFailedDelivery (boolean shouldRedeliver, boolean newException, boolean handled, boolean continued, boolean isDeadLetterChannel, Exchange exchange, String message, Throwable e)
specifier|private
name|void
name|logFailedDelivery
parameter_list|(
...
condition|)
block|{
comment|// do not log retry attempt because it is excluded by the retryAttemptedLogInterval
return|return;
block|}
block|}
else|else
...
condition|)
block|{
comment|// log intended rollback on maximum WARN level (not ERROR)
name|logger
operator|.
name|log
argument_list|(
...
return|;
block|}
comment|/**          * Determines whether or not to continue if we are exhausted.          *          * @param exchange the current exchange          * @return<tt>true</tt> to continue, or<tt>false</tt> to exhaust.          */
DECL|method|shouldContinue (Exchange exchange)
specifier|private
name|boolean
name|shouldContinue
parameter_list|(
...
return|;
block|}
comment|/**          * Determines whether or not to handle if we are exhausted.          *          * @param exchange the current exchange          * @return<tt>true</tt> to handle, or<tt>false</tt> to exhaust.          */
DECL|method|shouldHandle (Exchange exchange)
specifier|private
name|boolean
name|shouldHandle
parameter_list|(
...
return|;
block|}
comment|/**          * Method for sleeping during redelivery attempts.          *<p/>          * This task is for the synchronous blocking. If using async delayed then a scheduled thread pool          * is used for sleeping and trigger redeliveries.          */
DECL|method|sleep ()
specifier|public
name|boolean
name|sleep
...
name|InterruptedException
block|{
comment|// for small delays then just sleep
if|if
condition|(
name|redeliveryDelay
operator|<
...
condition|)
block|{
comment|// sleep using 1 sec interval
name|long
name|delta
init|=
name|redeliveryDelay
...
name|trace
argument_list|(
literal|"Sleeping for: {} millis until waking up for re-check"
argument_list|,
name|max
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|// are we preparing for shutdown then only do redelivery if allowed
if|if
condition|(
name|preparingShutdown
operator|&&
...
name|debug
argument_list|(
literal|"Rejected redelivery while stopping"
argument_list|)
expr_stmt|;
return|return
literal|false
