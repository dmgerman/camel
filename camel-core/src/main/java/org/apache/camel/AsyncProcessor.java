...
begin_comment
comment|/**  * An<b>asynchronous</b> processor which can process an {@link Exchange} in an asynchronous fashion  * and signal completion by invoking the {@link AsyncCallback}.  *<p/>  * Any processor can be coerced to have an {@link AsyncProcessor} interface by using the  * {@link org.apache.camel.impl.converter.AsyncProcessorTypeConverter#convert AsyncProcessorTypeConverter.covert}  * method.  *<p/>  *<b>Important:<b/> Use the {@link org.apache.camel.util.AsyncProcessorHelper#process(AsyncProcessor, Exchange, AsyncCallback)}  * method to invoke the process method, which ensure Camel have a chance to interweave and invoke it in a reliable manner.  * For example when using transactions all the invocations has to occur in synchronous manner to ensure the transaction  * work is done in the same thread, which is required by Spring TransactionMananger.  *  * @version $Revision$  */
end_comment
