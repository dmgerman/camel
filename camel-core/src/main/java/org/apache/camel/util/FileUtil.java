...
name|defaultTempDir
decl_stmt|;
comment|// current value of the "user.dir" property
DECL|field|gUserDir
specifier|private
specifier|static
name|String
name|gUserDir
decl_stmt|;
comment|// cached URI object for the current value of the escaped "user.dir" property stored as a URI
DECL|field|gUserDirURI
specifier|private
specifier|static
name|URI
name|gUserDirURI
...
DECL|field|gAfterEscaping1
specifier|private
specifier|static
name|char
name|gAfterEscaping1
index|[]
init|=
operator|new
name|char
...
DECL|field|gAfterEscaping2
specifier|private
specifier|static
name|char
name|gAfterEscaping2
index|[]
init|=
operator|new
name|char
...
block|}
decl_stmt|;
comment|// initialize the above 3 arrays
static|static
block|{
for|for
control|(
...
block|}
comment|// To escape the "user.dir" system property, by using %HH to represent
comment|// special ASCII characters: 0x00~0x1F, 0x7F, ' ', '<', '>', '#', '%'
comment|// and '"'. It's a static method, so needs to be synchronized.
comment|// this method looks heavy, but since the system property isn't expected
comment|// to change often, so in most cases, we only need to return the URI
comment|// that was escaped before.
comment|// According to the URI spec, non-ASCII characters (whose value>= 128)
comment|// need to be escaped too.
comment|// REVISIT: don't know how to escape non-ASCII characters, especially
comment|// which encoding to use. Leave them for now.
...
name|URISyntaxException
block|{
comment|// get the user.dir property
name|String
name|userDir
init|=
literal|""
...
parameter_list|)
block|{         }
comment|// return empty string if property value is empty string.
if|if
condition|(
name|userDir
operator|.
...
return|;
block|}
comment|// compute the new escaped value if the new property value doesn't
comment|// match the previous one
if|if
condition|(
name|gUserDirURI
operator|!=
...
return|;
block|}
comment|// record the new value as the global property value
name|gUserDir
operator|=
name|userDir
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// change C:/blah to /C:/blah
if|if
condition|(
name|len
operator|>=
...
block|}
block|}
comment|// for each character in the path
name|int
name|i
init|=
literal|0
...
argument_list|)
expr_stmt|;
comment|// if it's not an ASCII character, break here, and use UTF-8 encoding
if|if
condition|(
name|ch
operator|>=
...
argument_list|)
expr_stmt|;
comment|// record the fact that it's escaped
block|}
else|else
block|{
name|buffer
...
block|}
block|}
comment|// we saw some non-ascii character
if|if
condition|(
name|i
operator|<
...
condition|)
block|{
comment|// get UTF-8 bytes for the remaining sub-string
name|byte
index|[]
name|bytes
init|=
...
index|]
expr_stmt|;
comment|// for non-ascii character: make it positive, then escape
if|if
condition|(
name|b
operator|<
...
block|}
block|}
comment|// change blah/blah to blah/blah/
if|if
condition|(
operator|!
name|userDir
