...
condition|)
block|{
comment|// do nothing now as isValidFile() filters unmodified files
comment|/*             long fileModified = file.lastModified();             Long previousModified = noopMap.get(file);             noopMap.put(file, fileModified);             if (previousModified == null || fileModified> previousModified) {                 return 1;             }             else {                 if (LOG.isDebugEnabled()) {                     LOG.debug("Ignoring file: " + file + " as modified time: " + fileModified                               + " less than the previous modified time: " + previousModified);                 }                 return 0;             } */
block|}
else|else
block|{
if|if
