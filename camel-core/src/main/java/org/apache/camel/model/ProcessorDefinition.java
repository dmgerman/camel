...
argument_list|)
expr_stmt|;
comment|// add interceptor strategies to the channel
name|channel
operator|.
...
argument_list|)
decl_stmt|;
comment|// if the ProceedType/StopType create processor is null we keep on going
if|if
condition|(
operator|(
name|output
...
return|;
block|}
comment|/**      * Skip sending the {@link org.apache.camel.Exchange} to the original intended endpoint      * when used with<tt>interceptSendToEndpoint</tt>.      *      * @return the builder      */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
name|IllegalArgumentException
argument_list|(
literal|"Cannot use skip() without being within an interceptSendToEndpoint() block"
argument_list|)
throw|;
block|}
block|}
