...
expr_stmt|;
block|}
comment|/*         for (Map.Entry<Endpoint<Exchange>, Processor> route : routes) {             Endpoint<Exchange> key = route.getKey();             assertEquals("From endpoint", "seda:a", key.getEndpointUri());             Processor                     processor = route.getValue();             LoggingErrorHandler loggingProcessor =                     assertIsInstanceOf(LoggingErrorHandler.class, processor);         } */
block|}
specifier|public
name|void
...
comment|/*         Map<Endpoint,                 Processor> routeMap = builder.getRouteMap();         log.debug(routeMap);         Set<Map.Entry<Endpoint,                 Processor>> routes = routeMap.entrySet();         assertEquals("Number routes createdÊ", 2, routes.size());         for (Map.Entry<Endpoint,                 Processor> route : routes) {             Endpoint key = route.getKey();             String endpointUri = key.getEndpointUri();             Processor processor =                     route.getValue();             if (endpointUri.equals("seda:a")) {                 LoggingErrorHandler                         loggingProcessor = assertIsInstanceOf(LoggingErrorHandler.class,                         processor);                 Processor outputProcessor = loggingProcessor.getOutput();                 SendProcessor sendProcessor = assertIsInstanceOf(SendProcessor.class,                         outputProcessor);             }             else {                 assertEquals("From endpoint", "seda:b",                         endpointUri);                 DeadLetterChannel deadLetterChannel =                         assertIsInstanceOf(DeadLetterChannel.class, processor);                 Processor                         outputProcessor = deadLetterChannel.getOutput();                 SendProcessor sendProcessor = assertIsInstanceOf(SendProcessor.class, outputProcessor);         } */
block|}
...
comment|/*         Map<Endpoint,                 Processor> routeMap = builder.getRouteMap();         Set<Map.Entry<Endpoint,                 Processor>> routes = routeMap.entrySet();         assertEquals("Number routes created ", 1, routes.size());         for (Map.Entry<Endpoint,                 Processor> route : routes) {             Endpoint key = route.getKey();             assertEquals("From endpoint", "seda:a", key.getEndpointUri());             Processor                     processor = route.getValue();             DeadLetterChannel deadLetterChannel =                     assertIsInstanceOf(DeadLetterChannel.class, processor);             Endpoint                     deadLetterEndpoint = assertIsInstanceOf(Endpoint.class,                     deadLetterChannel.getDeadLetter());             assertEndpointUri(deadLetterEndpoint,                     "seda:errors");         } */
block|}
