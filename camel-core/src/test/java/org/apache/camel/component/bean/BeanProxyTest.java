...
expr_stmt|;
block|}
DECL|method|testBeanProxyVoidAsInOnly ()
specifier|public
name|void
name|testBeanProxyVoidAsInOnly
parameter_list|()
throws|throws
name|Exception
block|{
comment|// let void be InOnly which is a fire and forget method
name|OrderService
name|service
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// since its a fire and forget it should return faster so we can send
comment|// a Bye World message to the mock which arrives before the Hello World msg
name|template
operator|.
name|sendBody
argument_list|(
