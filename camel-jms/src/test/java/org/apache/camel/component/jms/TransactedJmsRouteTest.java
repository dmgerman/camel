...
argument_list|)
expr_stmt|;
comment|//		        JmsEndpoint endpoint = (JmsEndpoint)endpoint("activemq:queue:e");
comment|//		        from(endpoint).policy(catchRollback).policy(notsupported).policy(rollback).to("activemq:queue:mock.a");
block|}
block|}
return|;
block|}
