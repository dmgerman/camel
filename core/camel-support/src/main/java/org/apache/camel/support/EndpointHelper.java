...
expr_stmt|;
block|}
comment|/**      * Matches the endpoint with the given pattern.      *<p/>      * The endpoint will first resolve property placeholders using {@link CamelContext#resolvePropertyPlaceholders(String)}.      *<p/>      * The match rules are applied in this order:      *<ul>      *<li>exact match, returns true</li>      *<li>wildcard match (pattern ends with a * and the uri starts with the pattern), returns true</li>      *<li>regular expression match, returns true</li>      *<li>exact match with uri normalization of the pattern if possible, returns true</li>      *<li>otherwise returns false</li>      *</ul>      *      * @param context the Camel context, if<tt>null</tt> then property placeholder resolution is skipped.      * @param uri     the endpoint uri      * @param pattern a pattern to match      * @return<tt>true</tt> if match,<tt>false</tt> otherwise.      */
...
name|matchPattern
argument_list|(
name|toggleUriSchemeSeparators
argument_list|(
name|uri
argument_list|)
argument_list|,
...
condition|)
block|{
comment|// try normalizing the pattern as a uri for exact matching, so parameters are ordered the same as in the endpoint uri
try|try
block|{
name|pattern
operator|=
...
argument_list|)
expr_stmt|;
comment|// try both with and without scheme separators (//)
name|match
operator|=
name|toggleUriSchemeSeparators
argument_list|(
...
parameter_list|)
block|{
comment|//Can't normalize and original match failed
return|return
literal|false
return|;
block|}
...
return|;
block|}
comment|/**      * Toggles // separators in the given uri. If the uri does not contain ://, the slashes are added, otherwise they are removed.      * @param normalizedUri The uri to add/remove separators in      * @return The uri with separators added or removed      */
DECL|method|toggleUriSchemeSeparators (String normalizedUri)
specifier|private
specifier|static
name|String
name|toggleUriSchemeSeparators
