...
name|watch
decl_stmt|;
DECL|field|idleExecutorService
specifier|private
specifier|volatile
name|ScheduledExecutorService
name|idleExecutorService
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// use thread to stop Camel as otherwise we would block current thread
name|Thread
name|thread
init|=
name|camelContext
...
name|newThread
argument_list|(
literal|"CamelMainShutdownCamelContext"
argument_list|,
operator|new
name|ShutdownTask
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// shutdown idle checker if in use as we are stopping
if|if
condition|(
name|idleExecutorService
operator|!=
