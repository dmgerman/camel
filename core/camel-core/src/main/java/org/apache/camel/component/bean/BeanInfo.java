...
block|}
block|}
DECL|method|introspect (Class<?> clazz, MethodsFilter filteredMethods)
specifier|private
name|void
name|introspect
parameter_list|(
...
parameter_list|)
block|{
comment|// get the target clazz as it could potentially have been enhanced by
comment|// CGLIB etc.
name|clazz
operator|=
name|getTargetClass
...
expr_stmt|;
block|}
comment|// remove all abstract methods
if|if
condition|(
name|localOperationsWithBody
operator|!=
...
condition|)
block|{
comment|// do we have hardcoded parameters values provided from the method name then use that for matching
name|String
name|parameters
init|=
name|StringHelper
...
name|trace
argument_list|(
literal|"Choosing best matching method matching parameters: {}"
argument_list|,
name|parameters
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// if its a simple language then we need to evaluate the expression
comment|// so we have the result and can find out what type the parameter actually is
if|if
condition|(
name|StringHelper
operator|.
...
name|trace
argument_list|(
literal|"Evaluating simple expression for parameter #{}: {} to determine the class type of the parameter"
argument_list|,
name|index
argument_list|,
name|parameter
...
name|ExpressionBuilder
operator|.
name|attachmentObjectsExpression
argument_list|()
return|;
block|}
elseif|else
...
condition|)
block|{
comment|// use the default factory
name|type
operator|=
name|DefaultAnnotationExpressionFactory
operator|.
...
argument_list|()
decl_stmt|;
comment|// if the class is an interface then keep the method
name|boolean
name|isFromInterface
init|=
name|Modifier
operator|.
name|isInterface
...
condition|)
block|{
comment|// we cannot invoke an abstract method
name|it
operator|.
name|remove
argument_list|()
