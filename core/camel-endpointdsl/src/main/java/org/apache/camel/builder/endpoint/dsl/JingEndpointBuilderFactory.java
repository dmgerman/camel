...
argument_list|)
DECL|interface|JingEndpointBuilderFactory
specifier|public
interface|interface
...
block|}
block|}
comment|/**      * Jing (camel-jing)      * Validates the payload of a message using RelaxNG Syntax using Jing      * library.      *       * Category: validation      * Available as of version: 1.1      * Maven coordinates: org.apache.camel:camel-jing      *       * Syntax:<code>jing:resourceUri</code>      *       * Path parameter: resourceUri (required)      * URL to a local resource on the classpath or a full URL to a remote      * resource or resource on the file system which contains the schema to      * validate against.      */
