...
begin_comment
comment|/**  * The cql component aims at integrating Cassandra 2.0 using the CQL3 API (not  * the Thrift API). It's based on Cassandra Java Driver provided by DataStax.  *   * Generated by camel-package-maven-plugin - do not edit this file!  */
end_comment
...
name|LOCAL_ONE
block|;     }
comment|/**      * Cassandra CQL (camel-cassandraql)      * The cql component aims at integrating Cassandra 2.0 using the CQL3 API      * (not the Thrift API). It's based on Cassandra Java Driver provided by      * DataStax.      *       * Category: database,nosql      * Since: 2.15      * Maven coordinates: org.apache.camel:camel-cassandraql      *       * Syntax:<code>cql:beanRef:hosts:port/keyspace</code>      *       * Path parameter: beanRef      * beanRef is defined using bean:id      *       * Path parameter: hosts      * Hostname(s) cassansdra server(s). Multiple hosts can be separated by      * comma.      *       * Path parameter: port      * Port number of cassansdra server(s)      *       * Path parameter: keyspace      * Keyspace to use      */
DECL|method|cql (String path)
specifier|default
name|CassandraEndpointBuilder
name|cql
parameter_list|(
