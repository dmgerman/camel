...
block|{
comment|/**      * Aggregates an old and new exchange together to create a single combined exchange      *<p/>      * Important: In the aggregate method, do not create a new exchange instance to return,      * instead return either the old or new exchange from the input parameters; favor returning the old exchange whenever possible.      *      * @param oldExchange the oldest exchange (is<tt>null</tt> on first aggregation as we only have the new exchange)      * @param newExchange the newest exchange (can be<tt>null</tt> if there was no data possible to acquire)      * @return a combined composite of the two exchanges, return either the old or new exchange from the input parameters;      * favor returning the old exchange whenever possible)      */
