...
return|;
block|}
DECL|class|OsgiDefaultTypeConverter
specifier|private
class|class
name|OsgiDefaultTypeConverter
extends|extends
name|DefaultTypeConverter
block|{
DECL|method|OsgiDefaultTypeConverter (PackageScanClassResolver resolver, Injector injector, FactoryFinder factoryFinder, boolean loadTypeConverters)
specifier|public
name|OsgiDefaultTypeConverter
parameter_list|(
name|PackageScanClassResolver
...
parameter_list|)
block|{
comment|// favour keeping the converter that was loaded via TypeConverterLoader META-INF file
comment|// as OSGi loads these first and then gets triggered again later when there is both a META-INF/TypeConverter and META-INF/TypeConverterLoaded file
comment|// for the same set of type converters and we get duplicates (so this is a way of filtering out duplicates)
name|TypeConverter
name|converter
init|=
name|typeMappings
...
condition|)
block|{
comment|// the converter is already there which we want to keep (optimized via SimpleTypeConverter)
if|if
condition|(
name|converter
operator|instanceof
